...
### 7. 산출물 체크리스트
- [x] `CartReader` 클래스 (@Component) — `ProductFinder` 사용
- [x] `CartItemManager` 클래스 (@Component, @Transactional on methods)
- [x] 리팩토링된 `CartService` (@Service, 트랜잭션 없음)
- [x] 컨트롤러 입력 DTO의 `to***()` 변환 함수 적용
- [x] 응답 DTO의 `of(...)` 변환 적용
- [x] 단위/통합 테스트
...
### 8. 변경 범위(예상 파일)
- core/core-api/src/main/java/io/april2nd/commerce/core/api/controller/v1/CartController.java (호출부 정리)
- core/core-api/src/main/java/io/april2nd/commerce/core/domain/CartService.java (리팩토링)
- core/core-api/src/main/java/io/april2nd/commerce/core/domain/CartReader.java (신규) — `ProductFinder` 의존
- core/core-api/src/main/java/io/april2nd/commerce/core/domain/CartItemManager.java (신규)
- 테스트 경로 하 다수 파일
